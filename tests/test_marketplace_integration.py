...
class MarketplaceIntegrationTest:
    ...
    async def user_login(self):
        """Test user login to CWS"""
        logging.info(f"Testing user login with email {TEST_EMAIL}...")
        
        try:
            url = f"{CWS_URL}/auth/login"
            payload = {
                "email": TEST_EMAIL,
                "password": TEST_PASSWORD
            }
            
            logging.debug(f"Sending login request to {url}")
                ...
                try:
                    async with session.post(url, json=payload) as response:
                        if response.status == 200:
                            ...
                            logging.debug(f"Login response keys: {list(data.keys())}")
                            ...
                            if expiry:
                                try:
                                    self.user_token_expiry = datetime.fromisoformat(expiry)
                                except (ValueError, TypeError):
                                    self.user_token_expiry = datetime.now() + timedelta(days=7)
                            else:
                                self.user_token_expiry = datetime.now() + timedelta(days=7)
                        
                            # Save token
                            await self.save_user_token(self.user_token, self.user_token_expiry)
                            
                            logging.info(f"User login successful, token expires: {self.user_token_expiry}")
                            return True
                        else:
                            ...
                            logging.debug(f"Login request details: URL={url}, Payload={payload}")
                            return False
                except asyncio.TimeoutError:
                    logging.error(f"Login request timed out after 10 seconds")
                    return False
        except Exception as e:
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        try:
                            ...
                            logging.error(f"CWS decryption request timed out after 10 seconds")
                            return None
                except Exception as e:
...
async def _main():
    ...
    parser.add_argument('--skip-login', action='store_true', help='Skip login step (use existing token)')
    ...
    # Override test credentials if provided
    ...
    # Normal test flow
    # Test user login if needed and not skipped
    if not args.skip_login and (not test.user_token or (test.user_token_expiry and test.user_token_expiry <= datetime.now())):
        if not await test.user_login():
            logging.error("User login failed, cannot continue")
            return
