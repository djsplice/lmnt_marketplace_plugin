...
class HederaSlicer:
    def __init__(self, config):
        ...
        # Register the endpoint directly on the server with a valid prefix
    ...
    async def safe_cancel_print(self, klippy_apis):
        """Execute CANCEL_PRINT macro and ensure heaters are turned off."""
        try:
            ...
            try:
                ...
                logging.warning(f"Failed to execute RESTART: {str(restart_e)}")
            ...
            logging.error(f"Failed to execute CANCEL_PRINT macro: {str(cancel_e)}")
    ...
    async def stream_gcode_to_klipper(self, gcode_lines):
        """Background task to stream G-code lines to Klipper with optimized error handling."""
            ...
            for i, line in enumerate(gcode_lines):
                ...
                if line:  # Skip empty lines
                    try:
                        ...
                        # Removed the 50ms delay to increase streaming speed
                        #await asyncio.sleep(0.01)  # 50ms delay between commands
                    except Exception as e:
