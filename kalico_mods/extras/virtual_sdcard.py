...
class VirtualSDGCodeProvider:
    ...
    def get_gcode(self):
        ...
        while True:
            ...
            # After yield, check if a seek was requested. A seek is requested
            # if `set_file_position` was called, which changes `self.next_file_position`.
            # If no seek, `self.next_file_position` should still equal `self.file_position`
            # (its value from the start of this iteration).
            if self.next_file_position != self.file_position:
                # Seek requested. New position is in self.next_file_position.
                ...
                # No seek. Update positions from file using tell().
                try:
                    new_pos = self.current_file.tell()
                    self.file_position = new_pos
                    self.next_file_position = new_pos
                except (IOError, OSError):
                    logging.warning("virtual_sdcard: could not update position via tell()")
                    pass
    ...
    def load_file_from_handle(self, gcmd, file_handle, filename):
        # Reset internal state for the new file
        self.reset()
        try:
            file_handle.seek(0, os.SEEK_END)
            fsize = file_handle.tell()
            file_handle.seek(0)
        except Exception as e:
            logging.exception("virtual_sdcard handle seek")
            raise gcmd.error(f"Unable to access file handle: {e}")
        gcmd.respond_raw("File opened: %s Size: %d" % (filename, fsize))
        gcmd.respond_raw("File selected")
    ...
    def cmd_error(self, gcmd):
        raise gcode.error("SD write not supported")
...
class VirtualSD:
    """
    Custom VirtualSD class with support for encrypted GCode providers.
    This extends the standard Klipper VirtualSD to handle encrypted files
    stored in memory (memfd) without writing cleartext to disk.
    """
    def __init__(self, config):
        self.printer = config.get_printer()
        self.reactor = self.printer.get_reactor()
        ...
        self.encrypted_providers = {}  # Registry for encrypted file providers
        logging.info("Custom VirtualSD initialized.")

    def register_encrypted_provider(self, filename, provider):
        """Register a provider for an encrypted file."""
        self.encrypted_providers[filename] = provider
        logging.info(f"Registered encrypted provider for {filename}")
    ...
    def _load_file(self, gcmd, filename, check_subdirs=False):
        """
        Load a file for printing. Overrides the base method to check for
        encrypted providers before attempting filesystem access.
        """
        # Check if this is an encrypted file with a registered provider
        if filename.startswith("encrypted_") and filename in self.encrypted_providers:
            provider = self.encrypted_providers[filename]
            self.virtualsd_gcode_provider.current_file = provider
            self.virtualsd_gcode_provider.file_position = 0
            self.virtualsd_gcode_provider.file_size = provider.get_file_size()
            self.virtualsd_gcode_provider.filename = filename
            
            # Update print_stats for UI integration
            print_stats = self.printer.lookup_object('print_stats')
            print_stats.set_current_file(filename)
            print_stats.file_size = self.virtualsd_gcode_provider.file_size
            
            # Load metadata if available
            if hasattr(provider, 'metadata') and provider.metadata:
                if 'total_layers' in provider.metadata:
                    print_stats.total_layer = provider.metadata['total_layers']
                    gcmd.run_script_from_command(f"SET_PRINT_STATS_INFO TOTAL_LAYER={provider.metadata['total_layers']}")
                if 'filament_total' in provider.metadata:
                    print_stats.filament_total = provider.metadata['filament_total']
                    gcmd.run_script_from_command(f"SET_PRINT_STATS_INFO FILAMENT_TOTAL={provider.metadata['filament_total']}")
            
            gcmd.respond_raw(f"File opened: {filename} Size: {self.virtualsd_gcode_provider.file_size}")
