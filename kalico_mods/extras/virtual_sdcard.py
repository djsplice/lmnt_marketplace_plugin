...
class VirtualSDGCodeProvider:
    ...
    def load_file(self, gcmd, filename, check_subdirs=False, fd=None):
        if fd is not None:
            self.current_file = os.fdopen(fd, 'rb')
            logging.info(f"Using in-memory FD {fd} for G-code printing")
        else:
        ...
        self.file_size = os.fstat(self.current_file.fileno()).st_size
